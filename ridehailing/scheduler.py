...
def km_match(vehicles: List[Vehicle], orders: List[Order], max_distance_km: float = 20.0) -> List[Tuple[int,int]]:
    """
...
def nearest_match(vehicles: List[Vehicle], orders: List[Order], max_distance_km: float = 20.0) -> List[Tuple[int,int]]:
    pairs = []
    used_v = set()
    used_o = set()
    for j, o in enumerate(orders):
